...
function getListUrl() {
  ...
  const u = getJsonUrlForSlug(slug);
  return u || (typeof DEFAULT_URL !== "undefined" ? DEFAULT_URL : "");
}

function buildGameHref(id) {
  const slug = getSlugFromPath();
...
document.addEventListener("DOMContentLoaded", async () => {
  const slug = getSlugFromPath();
  const name = await resolveTranslatorNameFromRootIndex(slug);
  const label = name ? `f95list_${name}_viewer` : `f95list_viewer`;
  document.title = label;
  const h1 = document.querySelector("h1");
  if (h1) h1.textContent = label;

  // Si quelqu'un arrive sur /app/viewer directement, on affiche un message clair.
  if (!slug && location.pathname.startsWith("/app/")) {
    const host = document.querySelector(".top-title-row") || document.body;
    const p = document.createElement("div");
    p.style.padding = "10px";
    p.style.opacity = "0.9";
    p.innerHTML = "⚠️ Ouvre plutôt un traducteur via <b>/NomTraducteur/</b> (ex: <code>/ant28jsp/</code>).";
    host.appendChild(p);
  }
});
