...
// ===================== Nom affiché = texte du lien dans /index.html (racine) =====================
  ...
  async function resolveSiteDisplayNameFromRootIndex(){
    // Cherche : <a href="/ant28jsp/">Ant28jsp</a>
    try{
  ...
  async function setViewerTitles(){
    try{
      ...
      // retour vers la racine (liste des traducteurs)
      if (back) back.href = `/`;
    }catch{}
  }
...
(function(){
  ...
  if(!panel){
    ...
    panel.innerHTML = `
      ...
      <a class="btn" style="justify-content:flex-start; width:100%;" href="/${slug}/">📚 Viewer</a>
    `;
