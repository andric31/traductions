...
// ✅ Mode ALL : la liste est déjà fusionnée par viewer.js
let list = [];
try {
  if (window.__ALL_GAMES__ && Array.isArray(window.__ALL_GAMES__)) {
    list = window.__ALL_GAMES__;
  } else if (window.__ALL_DATA_PROMISE__) {
    const arr = await window.__ALL_DATA_PROMISE__;
    list = Array.isArray(arr) ? arr : [];
  } else {
    // fallback (au cas où) : comportement normal
    const listUrl = getListUrl();
    const raw = await fetchJson(listUrl);
    list = extractGames(raw);
  }
} catch (e) {
  // fallback (au cas où) : comportement normal
  const listUrl = getListUrl();
